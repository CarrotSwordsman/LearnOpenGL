...
int main(int argc, char *argv[])
{
    ...
    Shader shaderSingleColor("./shader/vertex.glsl", "./shader/stencil.glsl");
    ...
    unsigned int floorTexture = loadTexture("./static/texture/wall.jpg");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 渲染箱子
        glStencilFunc(GL_ALWAYS, 1, 0xFF); // 将所有片段的模板值更新为1
        ...
        // 通过模板渲染箱子边框
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);    // 只绘制比箱子大一圈的轮廓，该轮廓模板值仍未0
        glStencilMask(0x00);    // 禁用模板缓冲写入
        glDisable(GL_DEPTH_TEST);   // 禁用深度测试，让轮廓不被地面覆盖
        float scale = 1.1f;
        glBindVertexArray(cubeVAO);
        glBindTexture(GL_TEXTURE_2D, cubeTexture);
        model = glm::mat4(1.0f);
