...
int main(int argc, char *argv[])
{
    ...
    // 创建窗口对象
    GLFWwindow *window = glfwCreateWindow(800, 600, "LearnOpenGL-syc", NULL, NULL);
    if (window == NULL) {
    ...
    // note: 这里设置放大缩小的过滤方式为NEAREST，不做插值选择临近的值，得到的图片看到清晰像素
    ...
    // note: 预先设置片段着色器中uniform的值
    ourShader.setFloat("mixValue", mixValue);

    // 渲染循环
    while (!glfwWindowShouldClose(window))
    {
        // 有按键回调函数后就可以不用processInput函数了
        // processInput(window);
